...
class category
{
    //http://116.205.243.252:12345/category/list
    //{"token":"","uuid":""}
    public function list(Request $request)
    {
        ...
        $category_array = Db::table('category')->select('name')->get();

        if(is_null($category_array))
        {
            return json(['code'=>406,'message'=>'category_array is_null','data'=>array()]);
        }

        return json(['code'=>200,'message'=>'ok','data'=>$category_array]);
    }
}